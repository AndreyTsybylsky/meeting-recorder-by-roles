...
- **Email**: поддержана отправка письма нескольким получателям. Адреса можно вводить через запятую, точку с запятой, пробел или новую строку.

## [1.4] — 2026-04-24

### Добавлено

- **Google Meet: bootstrap captions при старте записи (one-shot)** в `content.js`:
  добавлена опциональная одноразовая попытка включить captions в начале сессии
  без циклического переоткрытия панели.

- **Popup: новые настройки captions UX** в `popup.html` и `popup.js`:
  добавлены переключатели для (1) скрытия нативных captions overlay и
  (2) одноразового bootstrap captions при старте записи.

### Изменено

- **Google Meet: ручной override видимости captions** в `content.js`:
  если пользователь вручную открывает captions, текст больше не скрывается
  extension-стилем; при ручном закрытии captions скрытие возвращается.

- **Google Meet: разделение capture/recovery и UI-видимости** в `content.js`:
  захват и восстановление потока не зависят от постоянных кликов по кнопке CC,
  что устраняет нежелательное автопереоткрытие captions.
